...
The first step is to define a grid over the study area. Here, we use an in-built bathymetry `SpatRaster` to represent the study area, but any grid is acceptable. `NA` defines inhospitable habitats (land in this case). Some movement and likelihood functions are implemented on a grid, so it is desirable to use a relatively fine resolution where possible.
...
To incorporate ancillary (e.g., archival) data, we may need to include additional columns in `obs` as required by the relevant likelihood functions. In our simulation, we'll account for the likelihood of depth observations using a uniform likelihood function provided by `patter` (`pf_lik_dc()`). Under this model, depth observations in sampled locations where the bathymetric depth lies between a lower and upper depth limit are equally likely, whereas observations outside those limits are impossible (Lavender et al., 2023). To implement this model, here we use a depth-error model to define lower and upper depth limits. In this example, we have to include a 'fudge' factor to account for the low resolution of the bathymetry data.
...
# (B) Define shallow and deep depth errors 
obs <-
  obs |> 
  mutate(
    depth_shallow_eps = cde(depth)[1, ],
    depth_deep_eps = cde(depth)[2, ]) |>
  as.data.table()
```
...
Particle samples can be used to reconstruct movement paths and build utilisation distributions. 
...
coord <- pf_coord(.history = out_pff$history, .bathy = gebco)
...
obs_acc <- obs[detection == 1L, ]
obs_acc <- lapply(split(obs_acc, obs_acc$timestep), function(d) {
  amat <- d$acoustics[[1]]
  data.table(timestep = d$timestep[1], 
             receiver_id = as.integer(names(amat)), 
             detection = as.integer(amat))
}) |> 
  rbindlist() |> 
  filter(detection == 1L) |> 
  mutate(timestamp = obs$timestamp[match(timestep, obs$timestep)]) |>
  as.data.table()
