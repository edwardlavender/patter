...
#' @param .png (optional) A named `list`, passed to [`grDevices::png()`], to save plots to file. Files are named `{.steps[1]}.png, {.steps[2]}.png, ..., {.steps[N]}.png`.
...
#' @param .prompt A `logical` variable that defines whether or not to prompt the user for input between plots. This is only used if `.png = NULL` (and there are multiple time steps).
#'
...
pf_plot_history <- function(.dlist,
  ...
  cl_lapply(.steps, .fun = function(t) {
    ...
    if (!is.null(.png)) {
      ...
      .png_defaults <- list(height = 10, width = 10, units = "in", res = 600)
