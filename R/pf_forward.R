...
#' @description These functions define selected function arguments for [`pf_forward()`] and associated routines.
#'
...
#'
#' @param .save,.sink,.cols [`pf_opt_record()`] arguments, passed to `.record`.
...
#' * `.sink`---a `character` string that defines a (usually) empty directory in which to write particles (as `.parquet` files).
...
#'    * For [`pf_forward()`], in general at a minimum you should retain `timestep`, `x_now`, `y_now` and `logwt`;
#'    * For other routines, you may require additional columns (TO DO);
#'    * To account for particle weights and to calculate effective sample size, `logwt` is required;
#'
#' At least one of `.save = TRUE` and `.sink` must be set.
#'
#' @param .drop,.sampler_batch_size [`pf_opt_control()`] arguments, passed to `.control`.
...
#'
#' @details These functions are defined separately for convenience of documentation. They do not define global options and must be passed to as arguments to relevant functions.
#'
#' @return
...
pf_opt_record <- function(.save = FALSE, .cols = NULL, .sink = NULL) {
  ...
  if (!is.null(.sink) && !is.null(.cols) && !("timestep" %in% .cols)) {
    abort("When `.sink` is provided, `.cols` must contain `timestep` to define file names.")
  }
  # Output list
  list(save = .save,
       cols = .cols,
...
#' @description This function implements the particle filter ('forward simulation'). The filter iteratively samples possible locations (particles) of an animal at each time point given the data up to (and including) that time point and a movement model.
#'
...
#'
#' @param .likelihood A named `list` of likelihood functions. These are used to calculate the log-likelihood of each dataset at proposal locations. See [`pf_lik`] for required arguments, convenience functions and advice.
...
#' @param .rerun,.rerun_from Rerun options. These options are used to restart the algorithm from an earlier time step.
...
#' 2. A likelihood step, in which we evaluate the log-likelihood of the data given each proposal.
...
#'
#' At the end of each time step, the ESS of weighted particles is evaluated. If the ESS is below the critical value specified in `.trial$trial_resample_crit` and/or directed sampling is implemented, particles are (re)-sampled, via `.sample`.
#'
#' The algorithm iterates over the time series, proposing, weighting and (re)sampling particles as described above. Particle samples can be saved in memory or written to file (with a speed penalty). If the function fails to convergence, a [`warning`] is returned alongside the outputs up to that time step. Otherwise, the function will continue to the end of the time series.
#'
#' # Algorithms
#'
...
#'
#' @return The function returns a [`pf_particles-class`] object. If `.record$sink` is specified, particles are written to file in a series of parquet files (`1.parquet, 2.parquet, ..., T.parquet`, where `T` is the number of time steps).
#'
#' @example man/examples/pf_forward-examples.R
#'
#' @seealso The particle filter samples locations (particles) that represent the possible locations of an individual through time, accounting for the data and the individual's movement.
#'
#' * To set up data, use [`pat_setup_data()`].
#'
#' * To implement the particle filter, use [`pf_forward()`]:
...
#'
#' * To map patterns of space use, use:
#'    * The backward smoother (TO DO);
#'    * [`pf_coord()`] to extract coordinates;
#'    * A `map_*()` function, such as [`map_pou()`], [`map_dens()`] and/or [`map_hr`]`_()`;
#'
#' * To reconstruct movement paths, use the backward sampler (TO DO).
#'
#' * For additional utilities, see supporting `pf_*()` functions, such as [`pf_files()`], [`pf_files_size()`], [`pf_plot_history()`] and [`pf_diag_summary()`].
#'
#' @author Edward Lavender
#' @export
