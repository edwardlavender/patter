...
#' * `.cols`---a `character` vector that defines the names of the columns in particle-sample [`data.table`]s to save and/or write to file at each time step. This reduces the space occupied by outputs. For [`pf_backward_killer()`], you need to retain `timestep`, `cell_past` and `cell_now`. For [`pf_backward_sampler`]`_*()`, you need `timestep`, `cell_now`, `x_now` and `y_now` for the backward sampler. For calculation of effective sample size, `lik` is required. `NULL` retains all columns.
#'
