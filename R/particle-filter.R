...
#'
#' See [`Patter.particle_filter()`](https://github.com/edwardlavender/Patter.jl) or `JuliaSwitch::julia_helpfile("particle_filter")` for further information.
#'
#' # Algorithms
#'
...
pf_filter <- function(.timeline,
  ...
  if (.collect) {
    ...
    out <- pf_particles(.pf_obj = pf_obj, .tzone = lubridate::tz(.timeline), .call_start = t1)
  } else {
    out <- nothing()
  }
  out
