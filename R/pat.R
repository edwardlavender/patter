...
#' @description This function is used to set up movement datasets, spatial datasets and associated parameters for all [`patter`] functions. In general, [`pat_setup_data()`] should be called before other routines.
#'
...
#' @param .lonlat (optional) A `logical` variable that defines whether spatial data (`.moorings` coordinates, `.bathy`) is in longitude/latitude format (`TRUE`) or planar (`FALSE`).
#'
#' @details
...
#'
#' If `.lonlat = TRUE`, downstream functions may require the `geosphere` package.
#'
...
#' * `algorithm`---an empty `list` that serves as a placeholder for algorithm layers required by [`pf_forward()`] and associated routines;
#'
#' @examples
