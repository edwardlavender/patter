...
#' @param .as_POSIXct (optional) A function that coerces supplied any supplied times (`.dlist$data$moorings$receiver_start`, `.dlist$data$$moorings$receiver_end`, `.dlist$data$services$service_start`, `.dlist$data$services$service_end`, `start` and `end`) that are not `POSIXct` objects to `POSIXct` objects. This can be suppressed via `as_POSIXct = NULL` if supplied times are Date objects and `delta_t` is not less than one day.
...
make_matrix_receivers <- function(.dlist,
  ...
  #### Check user inputs
  check_dlist(.obs, req = c("depth_shallow", "depth_deep"))(.dlist = .dlist, .dataset = "moorings")
