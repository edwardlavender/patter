...
#' * `.data$data$moorings`, with the following columns: `receiver_id`, `receiver_start`, and `receiver_end`.
...
#' @param .as_POSIXct (optional) A function that coerces supplied any supplied times (`.data$data$moorings$receiver_start`, `.data$data$$moorings$receiver_end`, `.data$data$services$service_start`, `.data$data$services$service_end`, `start` and `end`) that are not `POSIXct` objects to `POSIXct` objects. This can be suppressed via `as_POSIXct = NULL` if supplied times are Date objects and `delta_t` is not less than one day.
...
#'
#' @example man/examples/make_matrix_receivers-examples.R
...
make_matrix_receivers <- function(.data,
                                  .start = NULL,
                                  .end = NULL,
                                  .delta_t = "days",
                                  .as_POSIXct = NULL,
  ...
  #### Check user inputs
  check_data(.data = .data, .dataset = "moorings")
