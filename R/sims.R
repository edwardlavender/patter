...
#' @description These convenience functions support the generation of animal movement paths and observations in _de novo_ simulations (`sim_*()` functions) and simulation-based reconstructions of movement paths ([`pf_forward()`] and associates).
#'
...
#'
#' * **Correlated random walks.** It is possible to simulate correlated random walks in [`sim_path_walk()`] but at the time of writing this is not supported [`pf_forward()`]. Probability density functions for correlated random walks (e.g., `dangrw()`, `dangcrw()`) are not implemented.
#'
...
#'
#' * [`pf_forward()`] and associates implement the simulation-based reconstruction of movements;
