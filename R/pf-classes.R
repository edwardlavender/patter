...
pf_particles <- function(.pf_obj, .tzone, .call_start = NULL, call_end = Sys.time()) {
  ...
  if (!is.null(out$states)) {
    ...
    lubridate::tz(out$states$timestamp) <- .tzone
  }
  ...
  out$diagnostics <-
    out$diagnostics |>
    setDT()
  lubridate::tz(out$diagnostics$timestamp) <- .tzone
