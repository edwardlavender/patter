...
#'
#' The weights column (`weight`) is equal to the `lik` column, for particles generated by stochastic kicks, or the product of the `lik` and `dens` columns, in the case of directed sampling (see below). (Internally, `weights` are normalised to sum to one across particles for resampling in the `.sample` function (e.g., [`pf_sample_multinomial()`] or [`pf_sample_systematic()`].)
#'
